...
class DataProxyXmlLinkExtractor:
    ...
    def yield_all_dataproxy_links(self, response: HtmlResponse):
        ...
        for num in range(1, self.total_page + 1):
            ...
            yield Request(later_url, callback=self.yield_record_link,
                          meta={"url": later_url, "previous_url": response.url, "previous_content": response.text
                                })
    ...
    def yield_record_link(self, response):
        ...
        for url in record_urls:
            yield Request(url, meta={"url": url, "previous_url": response.url, "previous_content": response.text})
